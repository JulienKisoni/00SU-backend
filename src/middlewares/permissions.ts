...
import { ExtendedRequest, PERMISSION_LEVEL_KEYS, PermissionKey } from '../types/models';
import { createError } from './errors';
import { HTTP_STATUS_CODES } from '../types/enums';
import { roleActions } from 'src/permissions';
interface HasPerm {
  Model: PermissionKey;
...
export const hasPermission = ({ Model, Action }: HasPerm) => {
  return (req: ExtendedRequest<unknown>, _res: Response, next: NextFunction) => {
    const { user } = req;
